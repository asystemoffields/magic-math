...
CELLS = [
    ...
    md(
        ...
        "",
        "Now the loop from idea #6. The chart below updates live: **blue** is "
        "training loss (lower = less surprised by the next token), **orange** is "
        "validation loss on held-out text. Every so often the model writes a "
        "sample from the prompt *“Once upon a time”* — early ones are gibberish, "
        "later ones turn into sentences. That transition is the whole point.",
    ),
    code(
